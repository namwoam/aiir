...
a = a.astype(np.uint8)
b = np.repeat(a, 3)
c = np.zeros((64*8, 256*3, 3)).astype(np.uint8)
...
for vertical_line in range(c.shape[0]):
    color = vertical_line // (64*2)
    direction = (vertical_line // 64) % 2
    for rgb_channel in range(c.shape[2]):
        gradient = np.copy(b)
        if direction == 1:
            gradient = np.invert(gradient)
        if color == 0:
            c[vertical_line, :, rgb_channel] = gradient
        else:
            if color - 1 == rgb_channel:
                c[vertical_line, :, rgb_channel] = gradient
...
d = d**2
e = np.cumsum(d)
f = np.zeros((512, 512)).astype(np.float32)
...
def process(im_in, p):
    ...
    h = im_out.shape[0]
    if p == 0:
        im_out = np.rot90(im_in, axes=(0, 1))
    elif p == 1:
        im_out = np.flip(im_in, axis=1)
    elif p == 2:
        for y in range(h):
            for x in range(h):
                im_out[y][x] = np.roll(im_in[y][x], -1)
    elif p == 3:
        im_out = np.invert(im_in)
    elif p == 4:
        window_size = 8
        for y in range(window_size, h-window_size):
            for x in range(window_size, h-window_size):
                im_out[y][x] = np.sum(im_in[y-window_size:y+window_size+1, x -
                                      window_size:x+window_size+1], axis=(0, 1))/((2*window_size+1)**2)
    elif p == 5:
        im_out = (im_in // 64)*64
    return im_out
...
w = raw_image.shape[1]
h = min(h, w)
ch = raw_image.shape[2]
raw_image = raw_image[0:h, 0:h, :]
im3 = np.zeros((6, h, h, ch))
for i in range(6):
    im3[i] = process(raw_image, i)
...
for i, image in enumerate(images):
    fig3 = plt.figure()
    im4 = image.astype(np.float32)/255.0
    plt.imshow(im4)
    plt.get_current_fig_manager().canvas.setWindowTitle('Figure 3')
    plt.title(f"Random Order: {i+1}")
    plt.axis('off')
    plt.show(block=False)
    plt.pause(1)
    plt.close()
...
for i in range(6):
    im5 = im3[g[i]].astype(np.float32)/255.0
    x = i % 3
    y = i // 3
    ax[y, x].imshow(im5)
    ax[y, x].set_title(f'Proc. {g[i]}')
    ax[y, x].axis('off')
